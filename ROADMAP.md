...
| W21 | ⑥ 工程化 | 配置/日志/超时重试/限流/成本、pytest 基础；**+ MCP 协议（`03-3`，半格）** | 三个项目统一工程规范；一个自己写的 MCP server | 有测试、有日志、有错误兜底；能讲清 MCP 与 function calling 的区别 |
| W22 | ⑥ | **+ Ollama 本地模型（`01-2`，半格）**；Docker 基础 + 部署到一个公网可访问的地方 | 项目②或③公网链接 | 链接能打开、能用；本地模型能跑起来并说明「什么时候用本地、什么时候用 API」 |
...
| 时段 | 干什么 | 时长 |
|---|---|---|
| 工作日 | 主线任务（三步法） | 1.5-2h/天 |
| 周六 | 主线 + 收工提交 | 3-4h |
| 周日 | 主线收尾 + **泛读窗口**（见 `notes/README.md` 轨道 B）+ 10 分钟笔记复习 | 3-4h（其中泛读 30 分钟） |
...
概念定位与「什么不学」见 `notes/概念地图.md`；教程两条轨道见 `notes/README.md`。
